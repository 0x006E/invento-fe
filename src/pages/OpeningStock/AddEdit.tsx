...
export interface AddEditProps extends OmitStrict<ModalProps, "onSubmit"> {
  initialValues: OpeningStock;
  isAdd: boolean;
  isEdit?: boolean;
  onSubmit: (values: OpeningStock) => void;
}
...
function AddEdit(props: AddEditProps) {
  ...
  const handleFormSubmit = (values: OpeningStock) => {
  ...
  return (
    <Modal
      title={isAdd ? "Add OpeningStock" : "Edit OpeningStock"}
      centered
      {...rest}
      size="lg"
    >
          ...
          {!isEditWindow ? (
            active === 1 ? (
              ...
              <Button
                ...
                onClick={async (e) => {
                  e.preventDefault();
                  const result = await trigger(["partyId", "partyType"], {
                    shouldFocus: true,
                  });
                  if (result) nextStep();
                }}
