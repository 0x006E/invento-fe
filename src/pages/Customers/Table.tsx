...
export default function CustomersTable() {
  ...
  const { customers, add, remove, update } = useCustomers(pageSize);
  const { data, isFetching, isLoading, isError, refetch } = customers(
    page,
    debouncedQuery,
    sortStatus.columnAccessor as keyof Customer,
  ...
  const handleAdd = async (values: Customer) => {
    setOpen(false);
    addMutate(values, {
      onSuccess: () => {
        showNotification({
  ...
  const handleEdit = async (values: Customer) => {
    setOpen(false);
    updateMutate(values, {
      onSuccess: () => {
        showNotification({
    ...
    <>
      <Grid align="center" m={0} mb="xs">
        <Flex w={"100%"} gap={20}>
          ...
          <Button
            ...
            onClick={() => {
              setIsAdd(true);
              setInitialValues({ ...emptyCustomer });
              setOpen(true);
            }}
          >
      ...
      <Box sx={{ height: "60vh" }}>
        <DataTable<Customer>
          ...
          onRowClick={({ id, name, phoneNumber, address }) => {
            setInitialValues({ id, name, phoneNumber, address });
            setIsAdd(false);

            setOpen(true);
          }}
          rowContextMenu={{
            items: ({ id, name, phoneNumber, address }) => [
              {
                key: "edit",
